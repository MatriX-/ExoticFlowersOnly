...
TRUNCATE_MARKER = "FOR COA and MEDIA REFERENCE ONLY"

def get_target_sheet_id() -> Optional[str]:
    try:
        ...
        import os
        sheet_id_file = 'logs/target_sheet_id.txt'
        if os.path.exists(sheet_id_file):
            with open(sheet_id_file, 'r') as f:
                ...
                try:
                    ...
                    if metadata['properties']['title'] == TARGET_SHEET_NAME:
        ...
        try:
            ...
            results = drive_service.files().list(
                q=f"name='{TARGET_SHEET_NAME}' and mimeType='application/vnd.google-apps.spreadsheet' and trashed=false",
                spaces='drive',
...
def create_target_sheet() -> str:
    try:
        ...
        # Store the sheet ID for future updates
        import os
        os.makedirs('logs', exist_ok=True)
        with open('logs/target_sheet_id.txt', 'w') as f:
            f.write(sheet_id)
...
def copy_source_sheet() -> dict:
    try:
        ...
        for sheet in sheet_metadata.get('sheets', []):
            if sheet['properties']['title'] == SOURCE_SHEET_TAB:
                source_sheet = sheet
                break
        
        if not source_sheet:
            raise ValueError(f"Sheet tab '{SOURCE_SHEET_TAB}' not found in source spreadsheet")
        
        # Get values first to find truncation point
        values_result = sheets_service.spreadsheets().values().get(
            spreadsheetId=SOURCE_SHEET_ID,
            range=f"{SOURCE_SHEET_TAB}!A:Z",
            valueRenderOption='FORMATTED_VALUE'
        ).execute()
        
        values = values_result.get('values', [])
        
        # Find truncation row - try multiple variations of the marker
        truncate_markers = [
        ...
        sheet_data = sheets_service.spreadsheets().get(
            spreadsheetId=SOURCE_SHEET_ID,
            ranges=[f"{SOURCE_SHEET_TAB}!A1:Z{truncate_row}"],
...
def process_and_update_sheet(source_data: dict, target_sheet_id: str):
    try:
        sheets_service = get_sheets_service()
        ...
        logger.info(f"Processing {truncate_row} rows (truncated from {len(values)} total)")
        
        # Get the raw grid data to access all cell properties including hyperlinks
        source_grid_data = sheet_data['sheets'][0].get('data', [{}])[0]
        row_data = source_grid_data.get('rowData', [])[:truncate_row]  # ENFORCE truncation here
        ...
        # Prepare a wider canvas for the logo: merge D2:H2 and widen columns D..H
        ...
        # Row 2: Logo anchored at column D (index 3) with custom size
        ...
        # Rows 3-6: Contact information centered under the logo (merge D..H)
        contact_info = [
        ...
        for i, info in enumerate(contact_info):
            ...
            # Ensure the row is merged across D..H and idempotent on re-runs
            header_requests.append({
                'unmergeCells': {
                    'range': {
                        'sheetId': target_sheet_id_num,
        ...
        # Ensure header area (excluding contact rows and the logo row) has white background (clear any prior fills)
        ...
        # Set brand-blue background for the merged logo area D2:H2
        requests.append({
            'repeatCell': {
                'range': {
                    'sheetId': target_sheet_id_num,
        ...
        for row_index in range(truncate_row):  # EXPLICIT range to enforce truncation
            ...
            target_col = 0
            # Detect category from column B (TYPE column - index 1)
            if len(row['values']) > 1:
                type_text = _normalize(_cell_plain_text(row['values'][1]))
            ...
            for source_col, source_cell in enumerate(row['values']):
                # Skip columns A (0), F (5), G (6)
                if source_col in [0, 5, 6]:
                    continue
                ...
                # Apply category upcharge for price column (index 7 in source - "price per pound")
                if source_col == 7 and 'userEnteredValue' in source_cell and isinstance(source_cell['userEnteredValue'], dict):
                ...
                # Determine if the cell contains a hyperlink; also preserve any existing HYPERLINK formula
                ...
                requests.append({
                    'updateCells': {
                        'range': {
                            'sheetId': target_sheet_id_num,
                            'startRowIndex': row_index + header_rows,
                            'endRowIndex': row_index + header_rows + 1,
